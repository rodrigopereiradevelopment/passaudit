    ...
    match reqwest::blocking::get(url) {
        Ok(response) => {
            ...
            if body.to_uppercase().contains(&suffix.to_uppercase()) {
                println!(
                    "{}",
                    "⚠️ ATENÇÃO: Esta senha já foi encontrada em vazamentos públicos!"
                        .on_red()
                        .white()
                );
            } else {
                println!(
                    "{}",
                    "✅ Seguro: Nenhuma ocorrência encontrada nos vazamentos conhecidos.".green()
                );
            }
        }
...
fn generate_passphrase() {
    let words = vec![
        "alfa", "bravo", "codigo", "dados", "eco", "ferro", "pixel", "nuvem", "rede", "seguro",
        "terra", "vortex", "zero", "rust", "linux", "etec", "mogi", "arca", "cyber", "sentinel",
    ];
    ...
    println!(
        "{}",
        "Frases longas são mais fáceis de lembrar e muito mais seguras.".dimmed()
    );
}
